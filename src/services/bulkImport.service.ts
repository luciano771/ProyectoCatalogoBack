...
import { DEFAULT_NO_IMAGE_URL } from '../config/constants';
...
const HEADER_SYNONYMS: Record<string, (typeof FIELD_KEYS)[number]> = {
  nombre: 'Nombre',
  product: 'Nombre',
  producto: 'Nombre',
  articulo: 'Nombre',
...
function getColumnIndexMap(headers: string[]): Record<string, number> {
...
function parseActive(s: string): boolean {
  ...
  if (v === 'sí' || v === 'si' || v === 'yes' || v === '1' || v === 'true') return true;
...
function isHeaderRow(
  ...
  if (n === 'marca' || n === 'modelo' || n === 'nombre') return true;
  if (col['Marca'] !== undefined && col['Modelo'] !== undefined) {
    const marca = getCell(row, col['Marca']);
    const modelo = getCell(row, col['Modelo']);
    if (normalizeHeader(marca) === 'marca' && normalizeHeader(modelo) === 'modelo') return true;
  }
  return false;
}
...
export interface BulkImportInput {
  ...
  fileName: string;
  fileSize?: number;
  /** Si true, guarda el archivo Excel en la base (fileContent). Por defecto true. */
  storeFile?: boolean;
}
...
export async function bulkImportProducts(
  ...
  const genericImage =
    profile.bannerUrl && isValidUrl(profile.bannerUrl)
      ? profile.bannerUrl
  ...
  if (!firstSheet) throw AppError.validation('El archivo no contiene hojas');
  ...
  if (data.length < 2) {
    throw AppError.validation(
      'El archivo debe tener al menos la fila de encabezados y una fila de datos'
    );
  }
  ...
  if (!hasPrecio) {
    throw AppError.validation(
      'El archivo debe tener una columna de precio (Precio, CONTADO, PRECIO DE LISTA, etc.). Descargue la plantilla de ejemplo.'
    );
  }
  if (!hasNombre && !(hasMarca && hasModelo)) {
    throw AppError.validation(
      'El archivo debe tener columnas "Nombre" o bien "Marca" y "Modelo". Descargue la plantilla de ejemplo.'
    );
  }

  const firstDataCol =
    col['Nombre'] ?? col['Marca'] ?? 0;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const description =
      descripcion.length > 2000 ? descripcion.slice(0, 2000) : (descripcion || null);
  ...
  await prisma.bulkImport.create({
    data: {
      merchantProfileId: profile.id,
      ...
      errorsJson: result.errors.length > 0 ? (result.errors as object[]) : undefined
    }
  });
